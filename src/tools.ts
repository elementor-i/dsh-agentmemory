...
export type ParamSpec = { type: 'string' | 'number'; required: boolean; description: string }
...
const n = (description: string, required = false): ParamSpec => ({ type: 'number', required, description })
...
function pickDefined(args: unknown, params: Record<string, ParamSpec>): Record<string, unknown> {
...
export function buildTool(client: AgentMemoryClient, def: ToolDef, timeoutMs: number) {
...
export function buildHttpTool(client: AgentMemoryClient, timeoutMs: number) {
  return defineTool({
    name: 'memory_http',
    ...
    execute: async (args, exec) => {
      ...
      if (!path.startsWith('/agentmemory/')) throw new Error("memory_http: path must start with /agentmemory/")
      const method = (typeof a.method === 'string' && a.method.toUpperCase()) || 'POST'
      if (method !== 'GET' && method !== 'POST' && method !== 'DELETE') throw new Error('memory_http: method must be GET, POST, or DELETE')
      let body: Record<string, unknown> | undefined
      let query: Record<string, unknown> | undefined
      if (typeof a.body === 'string' && a.body.trim()) body = JSON.parse(a.body)
      if (typeof a.query === 'string' && a.query.trim()) query = JSON.parse(a.query)
      const result = method === 'GET'
        ? await client.request(path, { method: 'GET', query, timeoutMs, signal: exec.signal })
        : await client.request(path, { method: method as 'POST' | 'DELETE', body, timeoutMs, signal: exec.signal })
      return JSON.stringify(result, null, 2)
    },
  })
}
